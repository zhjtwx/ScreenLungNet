...
def train(train_loader, model, criterion, optimizer, epoch, logger, gpu_jitter=None, ema=None, lr_scheduler=None):
    ...
    for i, (input, label_mbs, dr_vector, img_dir, another_input) in enumerate(train_loader):

        with autocast():
            label_mb = label_mbs.long().view(-1)

            input = input.cuda(non_blocking=True)
            dr_vector = dr_vector.cuda(non_blocking=True)
            label_mb = label_mb.cuda(non_blocking=True)

            if gpu_jitter:
                input = gpu_jitter(input)
            input1_var = torch.autograd.Variable(input)
            input2_var = torch.autograd.Variable(dr_vector)
            label_mb_var = torch.autograd.Variable(label_mb)

            vit_cls_mb, mlp_cls, all_cls_mb = model(input1_var, input2_var)
...
def validation(val_loader, model, criterion, epoch, logger, optimizer, save_file):
    ...
    with torch.no_grad():
        for i,  (input, label_mbs, dr_vector, img_dir, another_input) in enumerate(val_loader):
